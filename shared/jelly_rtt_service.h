...
#define BT_UUID_JRS_VAL \
    BT_UUID_128_ENCODE(0x6f6a0001, 0x8e3d, 0x4d2f, 0xa3a1, 0xdeadbeef)
#define BT_UUID_JRS_PKT_VAL \
    BT_UUID_128_ENCODE(0x6f6a0002, 0x8e3d, 0x4d2f, 0xa3a1, 0xdeadbeef)
...
typedef struct __packed {
    uint8_t  ttl;        /* decrements at every hop */
    uint16_t dst;        /* destination ID (0xFFFF = broadcast) */
    uint16_t src;        /* source ID (e.g. node ID) */
    uint8_t  len;        /* number of valid bytes in payload[] */
    uint8_t  payload[12];/* useful data */
} jrs_pkt_t;
...
typedef void (*jrs_rx_cb_t)(const jrs_pkt_t *pkt);
